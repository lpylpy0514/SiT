...
from lib.models.ostrack import utils
...
class LeViT(torch.nn.Module):
    ...
    def __init__(self, img_size=224,
        ...
        self.conv1 = torch.nn.ConvTranspose2d(embed_dim[2], embed_dim[2], kernel_size=3, stride=2, padding=1)
        self.conv2 = torch.nn.ConvTranspose2d(embed_dim[1] + embed_dim[2], embed_dim[1] + embed_dim[2],
                                              kernel_size=2, stride=2)
        self.linear = torch.nn.Linear(sum(embed_dim), embed_dim[0])
    ...
    def forward(self, x):
        ...
        # x = self.blocks(x)
        x1 = self.blocks[0:self.fb_idx[0]](x)
        x2 = self.blocks[self.fb_idx[0]:self.fb_idx[1]](x1)
        x = self.blocks[self.fb_idx[1]:](x2)
        out_list = [x1, x2, x]
        for i in range(len(out_list)):
            B, N, C = out_list[i].shape
            HW = int(math.sqrt(N))
            out_list[i] = out_list[i].permute(0, 2, 1).view(B, C, HW, HW)
        ...
        # else:
        #     x = self.head(x)
        return self.feature_fusion(out_list)
