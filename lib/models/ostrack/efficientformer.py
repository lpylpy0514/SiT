...
class EfficientFormerV2(nn.Module):
    ...
    def forward(self, x):
        ...
        tracking_out = self.feature_fusion(x)
        # time4 = time.time()
        # print(f"time embed is :{(time2- time1)/(time4-time1)*100:.2f}%")
        # print(f"time forward_token is :{(time3 - time2)/(time4-time1)*100:.2f}%")
        # print(f"time fusion is :{(time4 - time3)/(time4-time1)*100:.2f}%")
