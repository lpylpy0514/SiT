...
class Feature_Mixer(nn.Module):
    def __init__(self, embed_dim=256, num_heads=6, mlp_ratio=4, qkv_bias=True, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., depth=6, norm_layer=None, act_layer= None, weight_init=''):
        super().__init__()
...
def Fearure_Mixer_base(embed_dim, depth=6, num_heads=6,**kwargs):
    model = Feature_Mixer(embed_dim=embed_dim, depth=depth, num_heads=num_heads, act_layer=nn.ReLU)
    return model